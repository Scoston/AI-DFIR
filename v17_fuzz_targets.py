...
PROFILE_NAMES = tuple(p.name for p in PROVIDERS) + tuple("archive-" + f for f in ARCHIVE_FORMATS) + ("docx-selected-parts", "html-static", "css-font-face") + structured.PROFILES
INSTRUMENTED_MODULES = (
    ...
    "v17_html_intake", "v17_structured_fuzz",
)
...
def exercise(data):
    """Selector 0..22 + raw bytes. Unknown selectors are ignored, never imported."""
    if not __debug__:
    ...
    def once():
        if selector >= 20:
            profile = structured.PROFILES[selector - 20]
            try:
                generated = structured.build(raw, profile=profile)
            except ValueError:
                return "REJECT", None
            status, digest = archives._outcome(generated, "zip") if profile == "zip-member-name" else docx_outcome(generated)
            if status == "REJECT": return status, None
            assert status == "ACCEPT"
            binding = {"profile": profile, "input_sha256": hashlib.sha256(raw).hexdigest(),
                       "generated_sha256": hashlib.sha256(generated).hexdigest(), "output_sha256": digest}
            return status, hashlib.sha256(canonical_json_bytes(binding)).hexdigest()
...
def curated_inputs():
    return tuple({**row, "data": bytes([PROFILE_NAMES.index(row["profile"])]) + row["payload"]}
                 for row in structured.load_corpus())
...
def preflight():
    ...
    with blocked_actions():
        ...
        for row in curated:
            status, digest = exercise(row["data"])
            if status != row["expected"]: raise AssertionError("curated seed expectation mismatch")
            curated_rows.append({"id": row["id"], "profile": row["profile"], "expected": row["expected"],
                                 "seed_sha256": hashlib.sha256(row["data"]).hexdigest(), "output_sha256": digest})
    return {"schema": SCHEMA, "status": "PASS", "profiles": len(rows), "cases": 2 * len(rows) + len(curated_rows),
            "seed_manifest_sha256": hashlib.sha256(canonical_json_bytes({"rows": rows, "curated": curated_rows})).hexdigest(),
            "curated_corpus_sha256": structured.CORPUS_SHA256, "curated_cases": len(curated_rows), "curated_rows": curated_rows,
