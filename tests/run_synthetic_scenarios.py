...
def main():
    ...
    o=unicode_analyze((s/'representation/unicode.md').read_text());assert 'unicode_tag' in types(o) and 'unicode_bidi' in types(o);r['unicode']='PASS'
    o=terminal_analyze((s/'representation/terminal_raw.log').read_text());assert 'terminal_cursor_or_display_control' in types(o) and 'terminal_osc52_clipboard_control' in types(o);r['terminal']='PASS'
    ...
    pol=readj(s/'agentic/router_policy.json');events=[json.loads(x) for x in (s/'agentic/router_events.jsonl').read_text().splitlines()];o=router_analyze(pol,events);assert 'unapproved_model_resolution' in types(o);r['router']='PASS'
    ...
    o=approval_analyze(readj(s/'approval/approval_records.json')['records']);assert 'approval_toctou_content_changed' in types(o);r['approval']='PASS'
    ev=[json.loads(x) for x in (s/'agentic/agentic_events.jsonl').read_text().splitlines()];o=causal_analyze(ev,[{'claim_id':'chain','source':'S','target':'C'}]);assert o['claims'][0]['supported'];r['causal']='PASS'
    ...
    final={'schema':'ai-dfir/synthetic-scenario-result/v1.6','status':'PASS','components':r};(HERE/'fixtures/SYNTHETIC_SCENARIO_RESULT.json').write_text(json.dumps(final,indent=2,sort_keys=True));print(json.dumps(final,indent=2,sort_keys=True))
