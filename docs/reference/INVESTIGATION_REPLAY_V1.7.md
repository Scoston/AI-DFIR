...
| `v17_log_analytics_context.normalize` | `1.7` | Native response, separate context artifact, and projection; metadata `{"input_format":"workspace-post","context_artifact_id":"QUERY-CONTEXT"}`; other explicit formats: `workspace-get`, `resource-post`, `resource-get` | Exact response/context binding plus complete typed result projection; context is a validated second lineage input; [GET](LOG_ANALYTICS_GET_CONTEXT_V1.7.md) and [resource](LOG_ANALYTICS_RESOURCE_V1.7.md) profiles bind their distinct scope/URL observations |
