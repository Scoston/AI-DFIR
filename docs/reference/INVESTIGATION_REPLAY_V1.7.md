...
| `v17_log_analytics.normalize` | `1.7` | Native query-result tables and recorded projection; metadata `{"input_format":"tables"}` or `resource-tables` | Canonical projection preserving raw digest, typed column/value bindings, partial-error state, and optional [resource permission observations](LOG_ANALYTICS_RESOURCE_V1.7.md) |
| `v17_log_analytics_lossless.normalize` | `1.7` | Native result and recorded lossless projection; metadata `{"input_format":"tables"}` or `resource-tables` | [Exact numeric text and wire kinds](LOG_ANALYTICS_LOSSLESS_V1.7.md), bounded wide values, typed token-tree digests and unchanged scope/completeness limits |
| `v17_log_analytics_context.normalize` | `1.7` | Native response, separate context artifact, and projection; metadata `{"input_format":"workspace-post","context_artifact_id":"QUERY-CONTEXT"}`; other formats: `workspace-get`, `resource-post`, `resource-get`, `workspace-get-form`, `resource-get-form` | Exact response/context binding plus complete typed result projection; context is a validated second lineage input; [GET](LOG_ANALYTICS_GET_CONTEXT_V1.7.md), [resource](LOG_ANALYTICS_RESOURCE_V1.7.md), and [form](LOG_ANALYTICS_GET_FORM_V1.7.md) profiles bind explicit scope/decoding observations |
