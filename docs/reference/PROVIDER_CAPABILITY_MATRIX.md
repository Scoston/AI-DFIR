...
| Google Cloud acquisition context (development) | [`google_cloud_logs --capture-context` and `gcp_logging_context_v17.py`](GCP_LOGGING_CAPTURE_V1.7.md) | One prepared entries.list request, exact bounded response bytes, separate signed context, and offline Audit Log projection replay | no automatic pagination or verified scope/origin; empty continuation pages stay incomplete and no-token pages stay unknown |
