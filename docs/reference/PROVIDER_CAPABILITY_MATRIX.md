...
| Retained Log Analytics request context (development) | [`log_analytics_context_v17.py`](LOG_ANALYTICS_CONTEXT_V1.7.md) | Separate signed request-context artifact bound to exact response digest/size and typed result projection | workspace POST only; context is a retained assertion, not proof of actual execution, effective scope, or completeness |
